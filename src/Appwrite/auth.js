...
import {setfileId,setDocId,setemailId,setuserId,setuserprofiledocumentId} from "../Redux/GetAvatarFileIDSlice.js"
let userId;
let fileID;
// let fileIDforFoodImage;

export const createUserAccount = async (email, password, username, name,dispatch) => {
...
export const updateForgetPasswordRecovery = async (userId, secret, newPassword) => {
  try {
...
export const saveFoodImageToStorage = async (title,foodimage,userId) => {
  ...
  const file = new File([blob], `food_image_${fileIDforFoodImage}.jpg`, { type: blob.type });
  ...
  try {
    ...
    const savedfoodDataToDB = await database.createDocument(
      ...
      DocumentID,
      {name:title,imageUrl:foodimage,userId:userId,dbId: DocumentID,        // Add these IDs to the document
    ...
    if (savedfoodPicToST && savedfoodDataToDB) {
      ...
      console.log("Food Image and data successfully get save to storage and database")
    }
  } catch (error) {
...
export const RetrieveFoodImageAndItsDataFromSTaDB = async (userId) => {
  try {
    ...
    if (listFoodDetails.documents.length > 0) {
      console.log("Saved meals retrieved successfully:", listFoodDetails.documents);
...
export const DeleteFoodImagesFromDataBase = async (databaseID,storageID) => {
  try {
    ...
    if(deleteDocumentfromdb && deleteImageFromStorage){
      console.log("Document and Image Deleted from Database and Storage successfully ")
      return true
    }
  } catch (error) {
    console.log("Error while deleting documents from db and storage:",error.message)
  }
}  
...
export const getUserAvatarWhileSignupAndSendingItToTheuserAvatarImageFromMainBucket = async(avatarUrl,userId) => {
  if (!userId || userId.length > 36 || !/^[a-zA-Z0-9._-]+$/.test(userId)) {
    console.error("Invalid userId:", userId);
    console.log("The userId is:",userId)
    return; // Prevent further execution
  ...
  try {
    ...
    if(uploadAvatarImageToStorage){
      console.log("Avatar Image Uploaded Successfully to userProfileCredentialBucket",uploadAvatarImageToStorage)
      return true
    }
  } catch (error) {
    console.log("Error while uploading avatar image to userProfileCredentialBucket",error.message)
  }
}
...
export const savedUserProfileSetupToDatabase = async (Phone,Location,Bio,userId,dispatch) => {
   ...
   try {
    ...
    if(ProfileInfoSaveToDB){
      console.log("User Information successfully get saved to database",ProfileInfoSaveToDB)
      dispatch(setuserprofiledocumentId(documetnId))
      return ProfileInfoSaveToDB
    }
   } catch (error) {
    console.log("Error while saving user profile information to database:",error.message)
   }
}
